...
>    ```
>    The `-ethkey` flag is **required** — it's the Ganache account private key (without `0x` prefix) used for on-chain identity registration when parents verify children through the dashboard.
...
func handleSignupSubmit(w http.ResponseWriter, r *http.Request) {
	...
	// Parse ring size (the extension sends the number of on-chain IDs as "decoy-size")
	var currentN int
	...
	// This MUST match the algorithm in bridge.go and cmd/client:
	//   N = 2 (u2sso.N), M = 10 (u2sso.M max)
	//   ringSize starts at 1, multiply by N each iteration until >= currentN
	currentM := 1
	ringSize := 1
...
func handleLoginSubmit(w http.ResponseWriter, r *http.Request) {
	...
	// The portable extension fills "veriqid-auth-proof" (content.js detects this element
	// to trigger the auth flow). Fall back to "veriqid-signature" for compatibility.
...
- **handleSignup / handleSignupSubmit** — The core ASC Verify() implementation. Generates challenge (message m), verifies ring membership proof (π) via `u2sso.RegistrationVerify()`, stores SPK (pseudonym/nullifier a) with UNIQUE constraint (Sybil-Resistance). Ring parameter `m` is calculated using the `N^m >= currentN` algorithm (matching the bridge and CLI). Debug logging prints all verification parameters.
- **handleLogin / handleLoginSubmit** — Auth flow using `u2sso.AuthVerify()` (Boquila signature verification). Reads from `veriqid-auth-proof` field first (portable extension), with fallback to `veriqid-signature`.
        ...
        <div class="auth-card">
            ...
            <form action="{{if .IsLogin}}/login{{else}}/signup{{end}}" method="POST" id="veriqid-form">
                <!-- ═══════════════════════════════════════════════
                     ...
                     The Phase 5 browser extension detects these by ID.
                     IDs must match exactly what content.js looks for.
                     ═══════════════════════════════════════════════ -->
                ...
                {{if .IsLogin}}
                <!-- Login: content script detects veriqid-auth-proof to trigger auth flow -->
                <input type="hidden" id="veriqid-auth-proof" name="veriqid-auth-proof" value="">
                {{else}}
                <!-- Signup: content script detects veriqid-proof to trigger registration flow -->
...
- The `id` attributes (`veriqid-challenge`, `veriqid-service-name`, `veriqid-proof`, `veriqid-auth-proof`, `veriqid-spk`, `veriqid-contract-index`, etc.) are what the content script scans for via `document.getElementById()`.
- **Fields are conditional on signup vs. login**: The signup form includes `veriqid-proof` (triggers the registration flow in `content.js`), while the login form includes `veriqid-auth-proof` (triggers the auth flow). This distinction is critical — if both are present, the content script may trigger the wrong flow.
...
- The content script also fills `veriqid-decoy-size` with the ring size (number of on-chain IDs). KidsTube reads this field for proof verification.
...
Terminal 3: go run ./cmd/veriqid-server -contract 0x<ADDR> -ethkey <GANACHE_PRIVATE_KEY> -service "KidsTube" -port 8080 -master-secret "my-hackathon-secret"
...
Shared signup/login template (controlled by `{{.IsLogin}}`). **Hidden fields are conditional**: signup forms include `veriqid-proof`, `veriqid-decoy-size`, `veriqid-nullifier`, and `veriqid-contract-index`; login forms include `veriqid-auth-proof` instead. This distinction is critical because the content script uses the presence of `veriqid-proof` vs. `veriqid-auth-proof` to determine whether to trigger the registration or auth flow. Visible fields: username input (signup only). Status indicator transitions from yellow "Waiting for extension..." to green "Verified!" when extension fills fields. Submit button starts disabled. Form POSTs to same URL (method-based routing).
...
## Integration Fixes Applied During Testing

The following changes were discovered and applied during end-to-end integration testing. They are already reflected in the code snippets above, but this section documents the reasoning for each fix.

### Fix 1: Bridge `msk_hex` Support for Identity Creation

**Problem:** The bridge's `/api/identity/create` endpoint only accepted `keypath` (file-based key). The portable extension uses mnemonic-derived keys via `msk_hex`, so creating identities from the extension flow required passing `msk_hex` directly.

**Fix (bridge/bridge.go):** Added `MskHex` field to `CreateIdentityRequest` and updated `HandleCreateIdentity` to accept hex-encoded MSK:

```go
type CreateIdentityRequest struct {
    Keypath    string `json:"keypath"`
    MskHex     string `json:"msk_hex,omitempty"`   // NEW: hex-encoded 32-byte MSK
...
The handler now checks: if `msk_hex` is provided, decode it directly (must be exactly 64 hex chars / 32 bytes); otherwise fall back to the original `keypath` flow.

### Fix 2: Veriqid Server Real On-Chain Registration

**Problem:** The parent dashboard server (`cmd/veriqid-server`) was only simulating on-chain registration — it logged "Simulating on-chain registration" instead of actually calling the contract. This meant parents could "verify" children but no identity was registered on-chain, so KidsTube's ring membership proof verification would fail.

**Fix:** Added a required `-ethkey` CLI flag to the Veriqid server. The `handleVerifyApprove` handler now calls `u2sso.AddIDstoIdR()` to register the child's MPK on-chain for real:
...
### Fix 3: Ring Parameter `m` Calculation Mismatch

**Problem:** KidsTube calculated the ring parameter `m` using bit-shifting (`floor(log2(N))`) which gave `m=2` for N=5, while the bridge and CLI used `N^m >= idsize` which gave `m=3`. This mismatch caused `RegistrationVerify` to always return false.

**Fix:** Replaced the bit-shift algorithm with the correct N^m >= idsize algorithm, matching bridge.go and cmd/client:
...
### Fix 4: `RegistrationVerify` Hardcoded Ring Size

**Problem:** The call to `u2sso.RegistrationVerify()` had `2` hardcoded as the third argument (ring size) instead of the actual `currentN`:

```go
// WRONG:
valid := u2sso.RegistrationVerify(proofHex, currentM, 2, ...)

// CORRECT:
valid := u2sso.RegistrationVerify(proofHex, currentM, currentN, ...)
```

### Fix 5: Content Script Not Filling `veriqid-decoy-size`

**Problem:** The content script (`extension-portable/content.js`) filled `veriqid-ring-size` but NOT `veriqid-decoy-size`. KidsTube reads `veriqid-decoy-size` to determine the ring size for proof verification, so it was always empty.

**Fix (extension-portable/content.js):** Added `decoySize` to the form field detection and fill logic:

```javascript
// In handleSignupAutoFill():
if (formInfo.fields.decoySize) {
    setFieldValue(formInfo.fields.decoySize, String(response.ring_size || ""));
}
```

### Fix 6: Login Form Detection by Content Script

**Problem:** The signup.html template had all hidden fields present for both signup and login modes. The content script uses the presence of `veriqid-proof` to detect signup forms and `veriqid-auth-proof` to detect login forms. With both present, the content script always triggered the signup flow, so login never worked.

**Fix:** Made the hidden fields conditional using Go template logic:

- **Signup** gets `veriqid-proof`, `veriqid-decoy-size`, `veriqid-nullifier`, `veriqid-contract-index`
- **Login** gets `veriqid-auth-proof`
- Both get `veriqid-challenge`, `veriqid-service-name`, `veriqid-spk`, `veriqid-signature`

### Fix 7: Login Handler Reading Wrong Field

**Problem:** The login handler read `veriqid-signature` but the portable extension fills `veriqid-auth-proof`. Result: login always failed with "Missing auth data."

**Fix:** Login handler now reads `veriqid-auth-proof` first, with fallback to `veriqid-signature` for compatibility:
...
### "Identity verification failed. The proof is invalid."

**Possible causes:**

1. **Ring parameter mismatch**: Ensure KidsTube's `m` calculation matches the bridge. Both must use the `N^m >= idsize` algorithm (not bit-shifting). See Fix 3 above.

2. **Hardcoded ring size in RegistrationVerify**: The third argument must be `currentN` (the actual ring size), not a hardcoded `2`. See Fix 4 above.

3. **Empty `veriqid-decoy-size` field**: The content script must fill this field. Check browser console for `[Veriqid Portable]` log messages. If `ring_size` is undefined in the response, the bridge may not be returning it. See Fix 5 above.

4. **Not enough identities on-chain**: Ring membership proofs require at least 2 identities. After redeploying Ganache (which is ephemeral), you must re-register identities:
   ```bash
   # Create dummy identities via bridge
   mkdir -p keys
   curl -X POST http://127.0.0.1:9090/api/identity/create -H "Content-Type: application/json" \
     -d '{"keypath":"keys/dummy1","ethkey":"<GANACHE_PRIVATE_KEY>","age_bracket":1}'
   curl -X POST http://127.0.0.1:9090/api/identity/create -H "Content-Type: application/json" \
     -d '{"keypath":"keys/dummy2","ethkey":"<GANACHE_PRIVATE_KEY>","age_bracket":1}'
   ```

5. **Stale challenge**: If a previous verification attempt failed, the challenge is already marked as used. Refresh the signup page to get a new challenge.

### "Challenge already used (replay attack prevented)"

The challenge was consumed by a previous (possibly failed) attempt. Simply refresh the signup page — this generates a new challenge.

### "Missing auth data. Is the Veriqid extension installed?"

**On the login page:**

1. Check that signup.html uses **conditional** hidden fields (login shows `veriqid-auth-proof`, signup shows `veriqid-proof`). See Fix 6 above.
2. Verify the content script detects the login form. Open browser console — you should see `[Veriqid Portable] Detected login form`.
3. Check that the login handler reads `veriqid-auth-proof` (not just `veriqid-signature`). See Fix 7 above.

### "Bridge is not running. Start the bridge server on port 9090."

The bridge server must be running before the extension can generate proofs. Start it:

```bash
cd /mnt/c/Users/patmekury/Downloads/Shape_Rotor/U2SSO/Veriqid
go build -o bridge-server ./cmd/bridge/
./bridge-server -contract 0x<CONTRACT_ADDRESS> -client http://127.0.0.1:7545
```

### "No identity key loaded. Open the Veriqid popup and paste your 12-word phrase."

The portable extension doesn't have an MSK stored. Open the extension popup and paste the 12-word mnemonic phrase that the parent received when verifying the child.

### Extension badge shows "OFF" (red)

The extension checks bridge health every minute. If the badge is red, the bridge is unreachable. Verify:
- Bridge is running on port 9090
- No firewall blocking localhost connections
- Check bridge console for errors

### After restarting Ganache, everything fails

Ganache is ephemeral — all on-chain data is lost on restart. You must:

1. Redeploy the contract: `cd contracts && truffle migrate --reset --network development`
2. Copy the new contract address
3. Restart bridge, Veriqid server, and KidsTube with the new address
4. Re-register at least 2 identities on-chain (dummy identities + the child's identity via parent dashboard)
5. Delete `kidstube.db` to clear stale user records: `rm kidstube.db`

### Login works but shows wrong user / "No account found"

The SPK is deterministic per (MSK, service_name). If you re-registered with a different MSK (new mnemonic), the SPK will be different from what's stored in `kidstube.db`. Delete the database and re-signup:

```bash
rm kidstube.db
# Restart KidsTube server, then sign up again
```
